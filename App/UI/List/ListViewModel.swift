...
class ListViewModel {
    ...
    private func createSection(_ items: ForecastItems) {
        let datas: [TableViewData] = items.map { item in
            ListCellData(forecast: item)
                ...
                .did(select: { [weak self] _ in self?.showDetail?(item) })
    ...
    private func trailingAction(item: ForecastItem) -> TableViewContextualAction {
        return TableViewContextualAction(title: "Delete", style: .destructive, backgroundColor: .red) { [weak self] _ in
            self?.service.remove(item: item)
        }
    }
}
